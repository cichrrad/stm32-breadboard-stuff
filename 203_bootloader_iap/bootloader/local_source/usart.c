...
{
    // 1. Enable Clocks (GPIOA, USART2, DMA1, DMAMUX)
    RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN;
    RCC->APB1ENR1 |= RCC_APB1ENR1_USART2EN;
    RCC->AHB1ENR |= RCC_AHB1ENR_DMA1EN | RCC_AHB1ENR_DMAMUX1EN;

    // 2. Configure PA2 (TX) and PA3 (RX) for Alternate Function 7 (USART2)
    GPIOA->MODER &= ~(GPIO_MODER_MODE2 | GPIO_MODER_MODE3);
    GPIOA->MODER |= (GPIO_MODER_MODE2_1 | GPIO_MODER_MODE3_1); // AF mode
    GPIOA->AFR[0] |= (7 << GPIO_AFRL_AFSEL2_Pos) | (7 << GPIO_AFRL_AFSEL3_Pos);

    // 3. Configure USART2 (115200 baud @ 16MHz)
    ...
    // 4. Configure DMA1 Channel 2 for RX
    ...
    // 5. Route USART2_RX (Request 26) to DMA1_Channel2 via DMAMUX
    DMAMUX1_Channel1->CCR = 26; // Channel 1 in DMAMUX corresponds to DMA1_CH2

    // 6. Enable USART2 Interrupt in NVIC (for IDLE line detection)
    NVIC_EnableIRQ(USART2_IRQn);
}

void USART2_DMA_Start(void)
{
    DMA1_Channel2->CCR |= DMA_CCR_EN; // Start listening!
}
